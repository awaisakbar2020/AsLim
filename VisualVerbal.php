...
{  
  ...
  { 
    ...
    $attempted_questions_using_attempt_id=$correctly_answered_graphical_ques=[];
      
    $intermediate_refined_ques_graphics=[];
    ...
    if (in_array($teachers_courses[$j], $courses))
    {
        ...
        for($z=0;$z<$len_question_id_Array;$z++)
        {
            ...
            {   
                $ques_graphics_id_Array[$q_g]=$row['id'];
            
                $q_g=$q_g+1;
            }
        }
        
        $total_graphical_ques_in_course=count($ques_graphics_id_Array);
        ...
        for($q5=0;$q5<$len_attempted_questions_using_attempt_id;$q5++)
        {
          ...
          {
              $intermediate_refined_ques_graphics[$q6]=$row['id'];
              
              $q6=$q6+1;
          }
            
        }
        $len_intermediate_refined_ques_graphics=count($intermediate_refined_ques_graphics);
        
        for($q7=0;$q7<$len_intermediate_refined_ques_graphics;$q7++)
        {
            if(in_array($intermediate_refined_ques_graphics[$q7],$questions_in_attempted_quizes))
            {
             $correctly_answered_graphical_ques[$p_s_q]=$intermediate_refined_ques_graphics[$q7];
             $p_s_q=$p_s_q+1;
            }
        }
        
        //$correctly_answered_graphical_ques gives all the attempted question. It is refined form
        
        $total_correctly_answered_graphical_ques=count($correctly_answered_graphical_ques);
        
        $s_q=$total_correctly_answered_graphical_ques/$total_graphical_ques_in_course;
        ...
        if($percentage>=75)
        {
            $hint_value_ques_graphics[$h]=3;
            $h=$h+1;
        }

        else if ($percentage>=50 AND $percentage<75)
        {
            $hint_value_ques_graphics[$h]=2; 
            $h=$h+1;
        }
        
        else if ($percentage>0 AND $percentage<50)
        {
            $hint_value_ques_graphics[$h]=1;
            $h=$h+1;
        }
  ...
  $n=0;
    ...
    for($m=0;$m<$len_teachers_courses;$m++)
    {
        ...
        while($n<$spliter[$m])
        {
           ...
           $add=$hint_value_content_visit[$n]+$hint_value_content_stay[$n]+$hint_value_forum_resource_posts[$n]+$hint_value_forum_resource_posts_reply[$n]+$hint_value_ques_graphics[$n]+$hint_value_ques_text[$n];
            
            $div=$add/6;
            
            $vis_verb_ls_dim[$n]=$div;
         
            $norm=($vis_verb_ls_dim[$n]-1)/2;
            ...
            {
                $LS[$n]="Visual";
                $desc[$n]="Student's behavior is giving strong indication towards Visual Learning Style";
            }
            else if ($normalized[$n]<0.5)
            {
                $LS[$n]="Verbal";
                $desc[$n]="Student's behavior is giving strong indication towards Verbal Learning Style";
            }
            else if($normalized[$n]==0.5)
            {
                $LS[$n]="Average/Balanced";
                $desc[$n]="Student’s behavior is average and therefore it is not giving a specific hint. Learning Style is neither Visual nor Verbal but Balanced";
            }
            
            
            echo '<tr><td>'.$inc.':   
            <td>'.$id_number.'</td><td></td><td>'. $short_name.'</td><td></td>
            <td>'.$full_name.'</td><td></td><td>'.$reg_no.'</td><td></td><td>'.$first_name.'</td><td></td><td>'.$last_name.'</td><td></td><td>'.$LS[$n].'</td><td></td>
            <td>'.$desc[$n].'</td></tr>';
            $inc=$inc+1;
            $n=$n+1;
        }
