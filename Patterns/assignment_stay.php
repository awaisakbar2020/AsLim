...
$result= mysqli_query($con,"SELECT id,userid,time FROM mdl_log WHERE module='assign' AND action='view submit assignment form'");
...
for($x=0;$x<$arrlength;$x++)
{
  ...
  while($module=='assign')
  {
      ...
      //user ids not equal case is useful when for example a user is attempting an assignment and at the same time another user logs in and views forum then that user id will be skipped and the code will find the user id which is equal to the initial user id (user whose stay we are going to find)
      ...
      while($row = mysqli_fetch_array($result2))
      {
          ...
          if($user_id==$uid)
          {
              echo "entered in user if statement";
              echo "<br>";
              if($module=='library' AND $action=='mailer')
              {
...
{
    echo"<br>";
    echo "Exercise stay of user id $final_useridArray[$f_s] is: ";
        echo $forum_stay[$f_s];
    echo"<br>";
}
