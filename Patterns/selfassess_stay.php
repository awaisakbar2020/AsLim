...
$result= mysqli_query($con,"SELECT id,userid,time FROM mdl_log WHERE module='quiz' AND action='attempt'");
...
for($x=0;$x<$arrlength;$x++)
{
  ...
  $uid=$useridArray[$x];
  $time1=$timeArray[$x];
  while($module=='quiz' OR $module=='grade')
  {
      ...
      while($row = mysqli_fetch_array($result2))
      {
          ...
          if($user_id==$uid)
          {
              if($module=='quiz' AND $action=='review')
              {
...
{
    echo"<br>";
    echo "Self assessment stay of user id $final_useridArray[$f_s] is: ";
        echo $forum_stay[$f_s];
    echo"<br>";
}
