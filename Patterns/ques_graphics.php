...
//-------------------------------------------------------------------------------------------------
                        //use generic course array here (may be session array)
//-------------------------------------------------------------------------------------------------
$q=0;
$ques=0;
$q_g=0;
$quiz_id_Array=[];
$question_id_Array=[];
$ques_graphics_id_Array=[];

$r1= mysqli_query($con,"SELECT id FROM mdl_quiz WHERE course='2'");
while($row = mysqli_fetch_array($r1))
{
  $quiz_id_Array[$q]=$row['id'];
  $q=$q+1;
}
$len_quiz_id_Array=count($quiz_id_Array);

for($i=0;$i<$len_quiz_id_Array;$i++)
{
   $r2= mysqli_query($con,"SELECT questionid FROM mdl_quiz_slots WHERE quizid=$quiz_id_Array[$i]");
   while($row = mysqli_fetch_array($r2))
    {
        $question_id_Array[$ques]=$row['questionid'];
        $ques=$ques+1;
    }
}
...
{
    $r3= mysqli_query($con,"SELECT id FROM mdl_question WHERE id=$question_id_Array[$z] AND qtype='ddimageortext'");
...
$len_ques_graphics_id_Array=count($ques_graphics_id_Array);
echo "Graphical questions are:";
echo "<br>";
for($i=0;$i<$len_ques_graphics_id_Array;$i++)
{
    echo "$ques_graphics_id_Array[$i]";
    echo "<br>";
}
//for($x=0;$x<$len_ques_graphics_id_Array;$y++)
//{
  //  echo "ques_graphics_id_Array";
    //echo "<br>";
//}
