...
$f_d_id= mysqli_query($con,"SELECT id,userid,time FROM mdl_log WHERE module='resource' AND action='view' AND info=$resource_id");
   
while($r2 = mysqli_fetch_array($f_d_id))
{
  $useridArray[$s]=$r2['userid'];
  $idArray[$s]=$r2['id'];
  $timeArray[$s]=$r2['time'];
  $s=$s+1;
}
...
{
    echo"<br>";
    echo "Outline stay of user id $final_useridArray[$f_s] is: ";
        echo $forum_stay[$f_s];
    echo"<br>";
}
