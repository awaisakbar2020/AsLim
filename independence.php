...
$teachers_courses=$AS_level=$desc=$percentage_ind=[];
...
for($j=0;$j<$len_teachers_courses;$j++)
{
  for($i=0;$i<$len_users;$i++)
  {
    ...
    if (in_array($teachers_courses[$j], $courses))
    {
        ...
        $r1=mysqli_query($con,"select id from mdl_resource where course=$teachers_courses[$j] AND (name LIKE 'Outline%' OR name LIKE '%Outline' OR name LIKE '%Outline%') AND  (name LIKE 'Chapter%' OR name LIKE '%Chapter' OR name LIKE '%Chapter%')");
...
{ 
  ...
  { 
    ...
    if (in_array($teachers_courses[$j], $courses))
    {
        ...
        $h3=$total_resources*4*$_SESSION['forum_content_visit_higher']; //number of visit in each forum during a week (handling here)
        
        $h2=$total_resources*2*$_SESSION['forum_content_visit_lower'];
...
<div id="CenterContent" class="content"><form action="AffectiveStates.php" method="POST"><input type="submit" name="Back" value="Back" class="glow"> <p class="narrative">to the selection of affective states</p></form><p id="maintitle">Affective States Calculation Module for Independence</p><h1 style="text-align: center;"></h1>
...
<table id="ListTable"><tbody><tr><th></th><th>Course Code</th><th></th><th>Shortname</th><th></th><th>Longname</th><th></th><th>Reg. No.</th><th></th><th>First Name</th><th></th><th>Last Name</th><th></th><th>Independence Level</th><th></th><th>Description</th></tr>
  ...
  $n=0;
    ...
    for($m=0;$m<$len_teachers_courses;$m++)
    {
        ...
        while($n<$spliter[$m])
        {
           ...
           $add=$hint_value_content_visit[$n]+$hint_value_outline_visit[$n]+$hint_value_example_visit[$n]+$hint_value_exercise_visit[$n]+$hint_value_selfasses_visit[$n]+$hint_value_forum_resource_posts[$n]+$hint_value_forum_resource_posts_reply[$n]+$hint_value_forum_resource_visit[$n]+$hint_value_assignment_revision[$n]+$hint_value_selfassess_revision[$n]+$hint_value_selfassess_stay[$n]+$hint_value_exercise_stay[$n];
            
            $div=$add/12;
            
            $independence[$n]=$div;
           
            $norm=($independence[$n]-1)/2;
            ...
            $percentage_ind[$n]=round(($normalized[$n]/1)*100,1);
            
            if($normalized[$n]>0.5)
            {
                $AS_level[$n]="High";
                $desc[$n]="Student's behavior is indicating that he is putting $percentage_ind[$n] % Independent";
            }
