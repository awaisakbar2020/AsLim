...
$confusion=$normalized=$percentage_confusion=$AS_level=$desc=[];
    ...
    if (in_array($teachers_courses[$j], $courses))
    {
        ...
        else
        {
           $s_q=$total_performed_selfasses_ques/$total_questions;
            ...
            else if ($percentage>=$_SESSION['self_assess_stay_lower'] AND $percentage<$_SESSION['self_assess_visit_higher'])
            {
...
{ 
  ...
  { 
    ...
    if (in_array($teachers_courses[$j], $courses))
    {
        ...
        $h3=$total_assignments*$_SESSION['forum_assign_post_higher'];
        
        $h2=$total_assignments*$_SESSION['forum_assign_post_lower'];
...
<div id="CenterContent" class="content"><form action="AffectiveStates.php" method="POST"><input type="submit" name="Back" value="Back" class="glow"> <p class="narrative">to the selection of affective states</p></form><p id="maintitle">Affective States Calculation Module for Confusion</p><h1 style="text-align: center;"></h1>
...
<table id="ListTable"><tbody><tr><th></th><th>Course Code</th><th></th><th>Shortname</th><th></th><th>Longname</th><th></th><th>Reg. No.</th><th></th><th>First Name</th><th></th><th>Last Name</th><th></th><th>Level of Confusion</th><th></th><th>Description</th></tr>
  ...
  $n=0;
    ...
    for($m=0;$m<$len_teachers_courses;$m++)
    {
        ...
        while($n<$spliter[$m])
        {
           ...
           $add=$hint_value_exercise_visit[$n]+$hint_value_example_stay[$n]+$hint_value_forum_resource_visit[$n]+$hint_value_forum_resource_posts_reply[$n]+$hint_value_selfasses_visit[$n]+$hint_value_content_stay[$n]+$hint_value_assignment_revision[$n]+$hint_value_forum_assignment_posts[$n];
            
            $div=$add/9;
            
            $confusion[$n]=$div;
          
            $norm=($confusion[$n]-1)/2;
            ...
            $percentage_confusion[$n]=round(($normalized[$n]/1)*100,1);
            
            if($normalized[$n]>0.5)
            {
                $AS_level[$n]="High";
                $desc[$n]="Student's behavior is indicating that he is $percentage_confusion[$n] % confused";
            }
